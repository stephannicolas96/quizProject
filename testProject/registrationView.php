...
<form action="#" method="POST">
    ...
    <h1><?= defined('registration') ? registration : 'Registration' ?></h1>   

    <label for="username" ><?= defined('username') ? username : 'Username' ?></label>
    <input id="username" type="text" placeholder="<?= defined(usernamePlaceholder) ? usernamePlaceholder : 'Enter an username' ?>" name="username" value="<?= !empty($savedUsername) ? $savedUsername : '' ?>" required />

    <label for="mail"><?= defined('mail') ? mail : 'Mail' ?></label>
    <input id="mail" type="text" placeholder="<?= defined('mailPlaceholder') ? mailPlaceholder : 'Enter a mail' ?>" name="mail" value="<?= !empty($savedMail) ? $savedMail : '' ?>" required />
    ...
    <p><?= defined('alreadyHaveAnAccount') ? alreadyHaveAnAccount : 'Already have an account?' ?><a href="loginView.php"><?= defined('signIn') ? signIn : 'Sign in' ?></a>.</p>
</form>
